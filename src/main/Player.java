...
public class Player {
	...
	public Player(TileMap tm) {
		...
		try {
			...
			idleSprites[0] = ImageIO.read(new File("idle.png"));
			jumpingSprites[0] = ImageIO.read(new File("jump.png"));
			fallingSprites[0] = ImageIO.read(new File("fall.png"));

			BufferedImage image = ImageIO.read(new File("walk.png"));
			for (int i = 0; i < walkingSprites.length; i++) {
				walkingSprites[i] = image.getSubimage(i * width + i, 0, width,
						height);
			}
